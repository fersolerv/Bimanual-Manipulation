...
int UR5_inv_kin(Transform transform, bool shoulder_positive, bool wrist_positive,
    ...
    if (in_interval(&c_beta,-1.,1.)) {
        ...
        theta_tmp[0] = alpha + (2.*shoulder_positive -1.)*beta + PI/2. - offset1;
    ...
    if (in_interval(&c_alpha,-1.,1.)) {
        ...
        theta_tmp[4] = (2.*wrist_positive-1.)*alpha -offset5;
    ...
    if (in_interval(&c_beta,-1.,1.)) {
        ...
        theta_tmp[1] = alpha +(2.*elbow_positive-1.)*beta - offset2;
...
void find_theta1(Transform transform, Solution *solution,
    ...
    if (in_interval(&c_beta,-1., 1.)) {
        ...
        theta1 = alpha + beta + PI/2. - offset1;
...
double saturate(double theta) {
    if (in_interval(&theta,-PI,PI)) {
        ...
        return theta - sign(theta)*2.*PI*floor(abs(theta)/(2.*PI) + 0.5);
    }
}
...
double distance(double *theta_a, double *theta_b) {
    double sq_sum = 0.;
    for (unsigned int i = 0; i < 6; ++i) {
        sq_sum += pow(theta_a[i]-theta_b[i],2.);
    }
    return sqrt(sq_sum);
}
